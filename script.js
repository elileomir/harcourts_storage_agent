...
// Webhook URL
const WEBHOOK_URL =
  "https://hup.app.n8n.cloud/webhook/80351e0b-d9ed-4380-81d7-c8f284e566f4/chat";
...
function updateStatus(status, message = "") {
  ...
  if (statusDot && statusText && statusMessage && header) {
    ...
    if (message) renderInlineStatus(message, status);
    else removeInlineStatus();
...
function renderInlineStatus(text, animStatus = "typing") {
  ...
  inlineStatusElement.innerHTML = `
        <span class="inline-status-text ${animStatus}">${escapeHtml(
    String(text)
  )}</span>
    `;
...
function handleUserInfoSubmit(e) {
  ...
  if (userInfo.name && userInfo.email) {
    ...
    addBotMessage(
      `Hello ${userInfo.name}! I'm here to help you with information about our storage units. How can I assist you today?`
    );
...
function addBotMessage(text, isTyping = false) {
  ...
  if (isTyping) {
    ...
    const innerHTML = /<[a-z][\s\S]*>/i.test(text)
      ? text
      : `<p>${escapeHtml(text)}</p>`;
    content = `
            <div class="message-avatar">
...
function handleVirtualTourRequest() {
  ...
  setTimeout(() => {
    ...
    addBotMessage(
      "Great! I'll show you our virtual tour gallery. Here you can see different storage unit options available."
    );
...
function handleWaitlistRequest() {
  ...
  setTimeout(() => {
    ...
    addBotMessage(
      "I'd be happy to help you join our waitlist! Please fill out the form below."
    );
...
function openWaitlistForm() {
  ...
  closeWaitlistModal();
  addBotMessage(
    "I've opened the waitlist form for you. Please fill it out to join our waitlist and we'll contact you when storage units become available."
  );
}
...
window.addEventListener("click", function (event) {
  ...
  const bookingDisclaimerModal = document.getElementById(
...
document
  .getElementById("virtualTourGallery")
...
function handleBookingRequest() {
  ...
  setTimeout(() => {
    ...
    if (!hasUserInfo) {
      addBotMessage(
        "I'd be happy to help you book a storage unit! First, please provide your name and email above, then I can help you with the booking process."
      );
      return;
    }
    ...
    if (hasExistingBooking) {
      ...
      addBotMessage(
        "Great! I'd be happy to help you book a storage unit. Please fill out the form below with your details."
      );
      setTimeout(() => {
        addBookingForm();
      }, 500);
...
function editExistingBooking() {
  ...
  expireActiveBookingForm();
  addBotMessage(
    "I'll help you edit your existing booking. Please update the details below."
  );
  setTimeout(() => {
                ...
                <form id="inlineBookingForm" onsubmit="handleInlineBookingSubmit(event)">
                    <div class="form-group">
                        ...
                        <select id="inlineFacility" name="facility" required>
                            ...
                            <option value="Deegan Marine" ${
                              existingData &&
                              existingData.facility === "Deegan Marine"
                                ? "selected"
                                : ""
                            }>Deegan Marine</option>
                            <option value="45 Fieldings Way" ${
                              existingData &&
                              existingData.facility === "45 Fieldings Way"
                                ? "selected"
                                : ""
                            }>45 Fieldings Way</option>
                            <option value="780 South Road" ${
                              existingData &&
                              existingData.facility === "780 South Road"
                                ? "selected"
                                : ""
                            }>780 South Road</option>
                        </select>
                    </div>
                    <div class="form-group">
                        <label for="inlineUnitNumber">Unit Number *</label>
                        <input type="text" id="inlineUnitNumber" name="unitNumber" required placeholder="Enter unit number" value="${
                          existingData ? existingData.unit : ""
                        }">
                    </div>
                    <div class="form-group">
                        <label for="inlineBondAmount">Bond Amount *</label>
                        <input type="number" id="inlineBondAmount" name="bondAmount" required placeholder="Enter bond amount" min="0" step="0.01" value="${
                          existingData ? existingData.bond : ""
                        }">
                    </div>
                    <div class="form-group">
                        <label for="inlineMonthlyAmount">Monthly Amount *</label>
                        <input type="number" id="inlineMonthlyAmount" name="monthlyAmount" required placeholder="Enter monthly amount" min="0" step="0.01" value="${
                          existingData ? existingData.monthly : ""
                        }">
                    </div>
                    <div class="form-group">
                        <label for="inlineLeaseStartDate">Lease Start Date *</label>
                        <input type="date" id="inlineLeaseStartDate" name="leaseStartDate" required value="${
                          existingData ? existingData.leaseStartDate : ""
                        }">
                    </div>
...
function expireActiveBookingForm() {
  if (activeBookingForm) {
    ...
    const formContainer = activeBookingForm.querySelector(
...
function showBookingDisclaimerModal(action = 'create') {
    const modal = document.getElementById('bookingDisclaimerModal');
    if (!modal) return;

    // Find the confirm button and update its onclick to pass the action
    const confirmBtn = modal.querySelector('.btn-primary');
    if (confirmBtn) {
        confirmBtn.setAttribute('onclick', `confirmBooking('${action}')`);
    }
...
function handleInlineBookingSubmit(e) {
    ...
    // Determine if this is a new booking or an edit
    const action = hasExistingBooking ? 'edit' : 'create';

    // Show disclaimer modal and pass the action to the confirm button
    showBookingDisclaimerModal(action);
}

function confirmBooking(action = 'create') {
...
async function sendBookingToWebhook(action = 'create') {
    if (!hasUserInfo) {
        addBotMessage(
            "Please provide your name and email first before we can continue our conversation."
        );
        return;
    }
    ...
    try {
        const message = action === 'edit' ? "I'm going to update my booking" : "I'm going to book";
        ...
        if (data && data.output && data.output.response) {
            ...
            addBotMessage(
                "I received an empty response for the booking. Please try again."
            );
        }
    } catch (error) {
        ...
        const randomMessage =
            aussieBookingMessages[
                Math.floor(Math.random() * aussieBookingMessages.length)
            ];
